...
#define SAMPST_RESPONSE_TIMES 1 /* sampst variable for response times. */ // Acumuladores estadisticos?
#define STREAM_THINK 1 /* Random-number stream for think times. */
#define STREAM_SERVICE 2 /* Random-number stream for service times. */ //Stream (SEMILLA) generar independientes variables SI SE PONEN DISTINTAS
 ...
 time. */
{
 ...
 transfer[1] = sim_time; //Armo los atributos de las listas usando transfer no me importa referenias la lista
 transfer[2] = expon(mean_service, STREAM_SERVICE);
 list_file(LAST, LIST_QUEUE); //archivo los atributos en la lista referente a la cola de ultimas
...
void start_CPU_run(void) /* Non-event function to start a CPU run of a job. */
{
 ...
 list_remove(FIRST, LIST_QUEUE); //Remover el progra de la cola y lo dejo en transfer
 ...
 list_file(FIRST, LIST_CPU); //ocupo a la CPU
...
void end_CPU_run(void) /* Event function to end a CPU run of a job. */
{
 ...
 terminal. */
 sampst(sim_time - transfer[1], SAMPST_RESPONSE_TIMES); //Estadiistica
 event_schedule(sim_time + expon(mean_think, STREAM_THINK),
 EVENT_ARRIVAL); //Programo la llegada del siguiente programa cuando ya regreso el finalizado al terminal
