...
float mean_interarrival, mean_service, length_doors_open; // 9-5 =480 min
FILE *infile, *outfile;
...
main() /* Main function. */
{
 ...
 while (list_size[LIST_EVENT] != 0) { // fin de las colas no hya a quien atender se acaba la sim
 ...
 depart((int) transfer[3]); /* transfer[3] is teller =>  teller es la ventanilla que queda libre
 number. */
 break;
 case EVENT_CLOSE_DOORS:
 event_cancel(EVENT_ARRIVAL); // cancelo llegadas cuando pasa el tiempo controlare el fin de la sim  con la cola restante
 break;
 }
 }
...
{
 int teller; //cajero en donde voy a encolar
 ...
 if (list_size[num_tellers + teller] == 0) { // es la lista de las colas + la ventanilla ejemplo 5 cajeros ventanilla3 hace referencia a la loitsa 8 del cajero
 ...
 EVENT_DEPARTURE); // este lleva a transfer 2 atributos para la llegada y pone el transfer3 en la lista 25 con el simtime
 ...
 case of ties). */
 shortest_length = list_size[1]; // partimos de la izaquierda para encontrara la mas corta a la izquierda
 shortest_queue = 1;
